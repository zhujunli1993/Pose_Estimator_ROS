...
from utils.data_augmentation import data_augment, data_augment_contrast
...
from utils_swin.basic_utils import fill_missing_clip
...
class NOCSDataSet(data.Dataset):
    def __init__(self, 
        ...
        prior_shape_path = os.path.join(self.data_dir,'obj_models/mean_points_emb.npy')
        prior_emb_path = os.path.join(self.data_dir,'obj_models/mean_embedding.npy')
        if mode == 'train':
            del img_list_path[2:]
        ...
        self.id2cat_name_CAMERA = {'1': '02876657', # bottle
                                   '2': '02880940', # bowl
                                   '3': '02942699', # camera
                                   '4': '02946921', # can
                                   '5': '03642806', # laptop
        ...
        self.prior_shape = np.load(prior_shape_path)
        self.prior_emb = np.load(prior_emb_path)
    ...
    def __getitem__(self, index):  
        ...
        if self.per_obj != '':
            if isinstance(gts['class_ids'], np.ndarray):
                idx = np.where(gts['class_ids']==self.per_obj_id)
                idx = idx[0][0]
            else:
        ...
        cam_scale = 1000.0
        depth = fill_missing_clip(depth, cam_scale, 1)

        
                                                            
        # from PIL import Image                                                   
        # array_normlized=255 * (depth - np.min(depth)) / (np.max(depth) - np.min(depth)) 
        # array_normlized = array_normlized.astype(np.uint8)
        # image = Image.fromarray(array_normlized, 'L')
        # image.save('/workspace/REPO/Clip_Pose/depth.png')
        ...
        # mask_img = Image.fromarray((mask_target*255).astype(np.uint8),'L')
        # mask_img.save('/workspace/REPO/Clip_Pose/mask.png')
        ...
        # np.savetxt('/workspace/REPO/Clip_Pose/pts.txt', pcl_in)
        ...
        data_dict['prior_shape'] = torch.as_tensor(self.prior_shape[cat_id], dtype=torch.float32).contiguous()
        data_dict['prior_emb'] = torch.as_tensor(self.prior_emb[cat_id], dtype=torch.float32).contiguous()
    ...
    def _sample_points(self, pcl, n_pts):
        ...
        if total_pts_num < n_pts:
            ...
            pcl = pcl[ids]
            # fps_samples_idx = fpsample.fps_sampling(pcl, 1024)
            # pcl = pcl[fps_samples_idx]
        return pcl
...
def process_batch_zero(batch_sample,
                  device,
                  pose_mode='quat_wxyz',
                  mini_batch_size=None,
                  PTS_AUG_PARAMS=None):
    
    assert pose_mode in ['quat_wxyz', 'quat_xyzw', 'euler_xyz', 'euler_xyz_sx_cx', 'rot_matrix', 'original'], \
    ...
    if PTS_AUG_PARAMS==None:
        ...
        model_da = batch_sample['model_point'].to(device)
        gt_s_da = batch_sample['fsnet_scale'].to(device)
    else:        
        PC_da, gt_R_da, gt_t_da, gt_s_da, model_da = data_augment(
    ...
    processed_sample = {}
    processed_sample['sym'] = batch_sample['sym_info'].to(device)
    # processed_sample['mean_shape'] = batch_sample['mean_shape'].to(device)
    processed_sample['pts'] = PC_da                # [bs, 1024, 3]
    # processed_sample['pts_color'] = PC_da          # [bs, 1024, 3]
    # processed_sample['model_pts'] = model_da
    ...
    processed_sample['gt_pose_scale'] = scale   # [bs, 4/6/3 + 3]
    # """ zero center """
    num_pts = PC_da.shape[1]
    zero_mean = torch.mean(PC_da[:, :, :3], dim=1)
    processed_sample['zero_mean_pts_1'] = copy.deepcopy(PC_da[:, :, :3])
    processed_sample['zero_mean_pts_1'][:, :, :3] -= zero_mean.unsqueeze(1).repeat(1, num_pts, 1)
    ...
    batch_sample['prior_shape'] = batch_sample['prior_shape'].to(device)
    num_pts_prior = batch_sample['prior_shape'].shape[1]
    zero_mean_prior = torch.mean(batch_sample['prior_shape'][:,:,:3],dim=1)
    processed_sample['zero_mean_prior'] = copy.deepcopy(batch_sample['prior_shape'][:, :, :3])
    processed_sample['zero_mean_prior'][:, :, :3] -= zero_mean_prior.unsqueeze(1).repeat(1, num_pts_prior, 1)
    batch_sample['prior_emb'] = batch_sample['prior_emb'].to(device)
    processed_sample['prior_emb'] = batch_sample['prior_emb']
...
def create_pose_candidates(rot, trans):
    rot_z_min, rot_z_max = rot[:,0].min(), rot[:,0].max()
    rot_y_min, rot_y_max = rot[:,1].min(), rot[:,1].max()
    rot_x_min, rot_x_max = rot[:,2].min(), rot[:,2].max()
    
    all_rz = torch.linspace(rot_z_min, rot_z_max, steps=10)
    all_ry = torch.linspace(rot_y_min, rot_y_max, steps=10)
    all_rx = torch.linspace(rot_x_min, rot_x_max, steps=10)
    
    trans_x_min, trans_x_max = trans[:, 0].min(), trans[:, 0].max()
    trans_y_min, trans_y_max = trans[:, 1].min(), trans[:, 1].max()
    trans_z_min, trans_z_max = trans[:, 2].min(), trans[:, 2].max()
    
    all_tx = torch.linspace(trans_x_min, trans_x_max, steps=10)
    all_ty = torch.linspace(trans_y_min, trans_y_max, steps=10)
    all_tz = torch.linspace(trans_z_min, trans_z_max, steps=10)
    
    pose_candidates = torch.ones((all_rz.shape[0]*all_ry.shape[0]*all_rx.shape[0]*
                                  all_tx.shape[0]*all_ty.shape[0]*all_ty.shape[0], 9))
    
    all_pose = torch.cartesian_prod(all_rz,all_ry,all_rx,all_tx,all_ty,all_tz)
    euler = all_pose[:,:3]
    rot_ = pytorch3d.transforms.euler_angles_to_matrix(euler,'ZYX')
    rot_ = pytorch3d.transforms.matrix_to_rotation_6d(rot_)
    trans_ = all_pose[:,3:]
    pose = torch.cat([rot_.float(), trans_.float()], dim=-1)
...
if __name__ == '__main__':
    cfg = get_config()
    cfg.per_obj = "camera"
    cfg.pose_mode = 'original'
    # cfg.pose_mode = 'euler_xyz'
    ...
    #         PTS_AUG_PARAMS=None
    #     )
    #     # pose_test.append(batch_sample['gt_pose'])
    #     pts_test.append(batch_sample['pts'])
    #     # z_pose_test.append(batch_sample['zero_mean_gt_pose'])
    #     # z_pts_test.append(batch_sample['zero_mean_pts'])
        
    # # pose_test = torch.cat(pose_test, dim=0)
    # pts_test = torch.cat(pts_test,dim=0)
    # # z_pose_test = torch.cat(z_pose_test, dim=0)
    # # z_pts_test = torch.cat(z_pts_test,dim=0)
    
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_test_pose.pt'
    # # torch.save(pose_test, save_pose_pth)
    
    # save_pts_pth = '/workspace/REPO/Clip_Pose/datasets/camera_test_pts.pt'
    # torch.save(pts_test, save_pts_pth)
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_test_z_pose.pt'
    # # torch.save(z_pose_test, save_pose_pth)
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_test_z_pts.pt'
    # # torch.save(z_pts_test, save_pose_pth)
    ...
    #         PTS_AUG_PARAMS=None
    #     )
    #     # pose_val.append(batch_sample['gt_pose'])
    #     pts_val.append(batch_sample['pts'])
    #     # z_pose_val.append(batch_sample['zero_mean_gt_pose'])
    #     # z_pts_val.append(batch_sample['zero_mean_pts'])
        
    # # pose_val = torch.cat(pose_val, dim=0)
    # pts_val = torch.cat(pts_val, dim=0)
    # # z_pose_val = torch.cat(z_pose_val, dim=0)
    # # z_pts_val = torch.cat(z_pts_val, dim=0)
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_val_pose.pt'
    # # torch.save(pose_val, save_pose_pth)
    # save_pose_pth = '/workspace/REPO/Clip_Pose/datasets/camera_val_pts.pt'
    # torch.save(pts_val, save_pose_pth)
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_val_z_pose.pt'
    # # torch.save(z_pose_val, save_pose_pth)
    # # save_pose_pth = '/workspace/GenPose/datasets/laptop_val_z_pts.pt'
    # # torch.save(z_pts_val, save_pose_pth)
    ...
    for index, batch_sample in enumerate(tqdm(train_loader)):
        ...
        pose_train.append(batch_sample['gt_pose'])
        pts_train.append(batch_sample['pts'])
        # z_pose_train.append(batch_sample['zero_mean_gt_pose'])
        # z_pts_train.append(batch_sample['zero_mean_pts'])
        
        
    # pose_train = torch.cat(pose_train, dim=0)
    pts_train = torch.cat(pts_train, dim=0)
    # z_pose_train = torch.cat(z_pose_train, dim=0)
    # z_pts_train = torch.cat(z_pts_train, dim=0)
    # save_pose_pth = '/workspace/GenPose/datasets/laptop_train_pose.pt'
    # torch.save(pose_train, save_pose_pth)
    save_pose_pth = '/workspace/REPO/Clip_Pose/datasets/camera_train_pts.pt'
    torch.save(pts_train, save_pose_pth)
    # save_pose_pth = '/workspace/GenPose/datasets/laptop_train_z_pose.pt'
    # torch.save(z_pose_train, save_pose_pth)
    # save_pose_pth = '/workspace/GenPose/datasets/laptop_train_z_pts.pt'
    # torch.save(z_pts_train, save_pose_pth)



    # pose = torch.cat((pose_train, pose_val, pose_test), dim=0)
    
    # # rot = pose[:, :3]
    # # trans = pose[:, 3:]

    # # sampled_pose = create_pose_candidates(rot, trans)
    # save_pose_pth = '/workspace/GenPose/datasets/laptop_pose.pt'
    # torch.save(pose, save_pose_pth)
