...
def PoseDataImg(depth_pth, mask_pth, label_pth, bbox_pth, intrinsic,pc_pth=None):
    def _get_depth_normalize( roi_depth, roi_m_d_valid):
    ...
    def _depth_to_pcl_map(depth, K, mask):
        K = K.reshape(-1)
        ...
        Y = (v - cy) * depth / fy
        Z = depth
        # Stack the coordinates and reshape to (N, 3)
    ...
    def _allPtx_to_pcl(pts, mask):
        
        # pts = pts.reshape(-1).astype(np.float)
        valid = ((pts > 0) * mask.reshape(-1)) > 0
        pts = pts[valid]
        
        return pts.astype(np.float32)
    cat_names = ['bottle', 'laptop']
    cat_name2id = {'bottle': 44, 'laptop': 73}
    id2cat_name = {'44': 'bottle', '73': 'laptop'}
    cat_ids = [44, 73]

    # cat_names = ['bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']
    cat_name2id_nocs = {'bottle': 1, 'bowl': 2, 'camera': 3, 'can': 4, 'laptop': 5, 'mug': 6}
    id2cat_name_nocs = {'1': 'bottle', '2': 'bowl', '3': 'camera', '4': 'can', '5': 'laptop', '6': 'mug'}
    # 
    n_pts = 1024
    img_size = 256
    
    all_bbox = np.load(bbox_pth)
    all_label = np.load(label_pth)
    # all_depth = cv2.imread(depth_pth)
    
    if pc_pth is not None:
        all_pts = np.load(pc_pth)
    all_depth = np.load(depth_pth)
    all_mask = np.load(mask_pth)
    all_depth[np.isnan(all_depth)] = 0.0
    
    im_H, im_W = all_depth.shape[0], all_depth.shape[1]
    
    num_instance = len(all_label)
    ...
    for j in range(num_instance):
        ...
        if cat_id not in cat_ids:
            continue
        cat_name = id2cat_name[str(cat_id)]
        nocs_cat_id = cat_name2id_nocs[cat_name]
        coord_2d = get_2d_coord_np(im_W, im_H).transpose(1, 2, 0)
        # aggragate information about the selected object
        mask = all_mask[j, :, :].squeeze() # (H, W)
        
        mask[mask>0.5]=1
        mask[mask<0.5]=0
        pcl_in = _depth_to_pcl_map(all_depth,intrinsic,mask)
        # save_pts = all_pts[mask==1]
        # np.savetxt('/workspace/clip/ContrastPose/modified_data/2_crop_pts_new.txt',save_pts)
        # from PIL import Image
        # img = Image.fromarray(mask.astype(np.uint8)*255)
        # img.save('/workspace/clip/ContrastPose/modified_data/target_msk.png')
        ...
        #     all_depth, bbox_center, scale, FLAGS.img_size, interpolation=cv2.INTER_NEAREST
        # )
        ...
        # roi_m_d_valid = roi_mask.astype(np.bool) * depth_valid # as our depth_valid is point cloud format (256*256*3)
        # if np.sum(roi_m_d_valid) <= 1.0:
        #     return None
        # pcl_in = _depth_to_pcl(roi_depth, intrinsic, roi_coord_2d, roi_mask)
        ############## Crop point clouds ################
        # import pdb;pdb.set_trace()
        # pts_valid = ((np.sum(roi_depth,axis=-1) > 0) * roi_mask) > 0
        # pcl_in = roi_depth[pts_valid].astype(np.float32)
        
        pcl_in = _sample_points(pcl_in, n_pts)

        # occupancy canonical
        # sym
        
        sym_info = get_sym_info(id2cat_name[str(cat_id)])
        mean_shape = get_mean_shape(id2cat_name[str(cat_id)])
        mean_shape = mean_shape / 1000.0
        sym_infos.append(sym_info)
        mean_shapes.append(mean_shape)
        obj_ids.append(nocs_cat_id)
        obj_ids_0base.append(nocs_cat_id - 1)
