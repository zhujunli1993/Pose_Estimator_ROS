...
from clip.CLIP_gpv_split_trans import CLIPModel_GCN_all as CLIPModel_trans
from clip.CLIP_gpv_split_rot import CLIPModel_HS_all as CLIPModel_rot
...
def draw_all(opt, result, save_dir):
    """ Load data and draw visualization results.
    """
    img_id = result["image_path"].split('/')[-1]
    scene_id = result["image_path"].split('/')[3]
    if result["image_path"].split('/')[1] == 'real':
        data_type = 'Real'
    ...
    img_source = result["image_path"].split('/')[2]
    img_pth = os.path.join(opt.dataset_dir, data_type, img_source, scene_id, img_id+"_color.png")
    ...
    for i, cls in enumerate(result["gt_class_ids"]):
        ...
        idx_pred = result["pred_class_ids"] == cls
        pred_RTs = result["pred_RTs"][idx_pred]
        pred_scales = result["pred_scales"][idx_pred]
        idx_gt = result["gt_class_ids"] == cls
        gt_RTs = result["gt_RTs"][idx_gt]
        gt_scales = result["gt_scales"][idx_gt]
        miss = False
        if len(pred_RTs) <= 0:
    ...
    draw_bbox(image, all_gt_RTs, all_pred_RTs, all_gt_scales, all_pred_scales, class_ids=result["gt_class_ids"], misses=misses, 
              intrinsics=intrinsics, save_path=os.path.join(save_dir, data_type+"_"+img_source+"_"+scene_id+"_"+ img_id+"_bbox.png"))
...
def evaluate(argv):
    ...
    t_inference = 0.0
    img_count = 0
    pred_result_save_path = os.path.join(output_path, 'pred_result_vis.pkl')
    if os.path.exists(pred_result_save_path):
        ...
        clip_model_rot = CLIPModel_rot().to(FLAGS.device)
        ...
        clip_model_t = CLIPModel_trans().to(FLAGS.device)
    ...
    for r in tqdm(pred_results[:2800], desc="draw"):
        draw_all(FLAGS, r, save_dir)
    print("Drawing Done!")
