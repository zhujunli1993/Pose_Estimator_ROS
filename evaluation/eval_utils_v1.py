    ...
    for result in progress:
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_overlaps = compute_RT_overlaps(cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility,
                                              ...
                                              synset_names)
            # if RT_overlaps.shape != (1,1,2) and RT_overlaps.shape != (0,0,2):
...
def draw_bbox(image, gt_RTs, pred_RTs, gt_scales, pred_scales, class_ids, misses, intrinsics, save_path="bbox.png"):
    
    def draw_img(gt_RT, pred_RT, gt_scale, pred_scale, class_name, save_path, draw_image):
        if class_name in ['bottle', 'bowl', 'can']:
            def y_rotation_matrix(theta):
                return np.array([[np.cos(theta), 0, np.sin(theta), 0],
                                    [0, 1, 0, 0],
                                    [-np.sin(theta), 0, np.cos(theta), 0],
            ...
            n = 20
            max_iou = 0
            pred_rotated_RT = pred_RT
            for i in range(n):
                new_rotated_RT = pred_RT@y_rotation_matrix(2*math.pi*i/float(n))
                new_iou = asymmetric_3d_iou(new_rotated_RT, gt_RT, pred_scale, gt_scale)
                if max_iou <= new_iou:
                    pred_rotated_RT = new_rotated_RT
                    max_iou = new_iou
                    
            noc_cube_pred = get_3d_bbox(pred_scale, 0)
            pred_bbox_3d = transform_coordinates_3d(noc_cube_pred, pred_rotated_RT)
            
            noc_cube_gt = get_3d_bbox(gt_scale, 0)
            gt_bbox_3d = transform_coordinates_3d(noc_cube_gt, gt_RT)
            ...
            noc_cube_pred = get_3d_bbox(pred_scale, 0)
            pred_bbox_3d = transform_coordinates_3d(noc_cube_pred, pred_RT)
        

        
        projected_axes_pred = calculate_2d_projections(
            pred_bbox_3d, intrinsics)
        projected_axes_gt = calculate_2d_projections(
            gt_bbox_3d, intrinsics)
        ...
        for edge in edges:
            pt1 = tuple(projected_axes_pred[edge[0]].astype(int))
            pt2 = tuple(projected_axes_pred[edge[1]].astype(int))
            cv2.line(draw_image, pt1, pt2, (255, 165, 0), 2)  # red: predict
        for edge in edges:
            pt1 = tuple(projected_axes_gt[edge[0]].astype(int))
            pt2 = tuple(projected_axes_gt[edge[1]].astype(int))
            cv2.line(draw_image, pt1, pt2, (0, 255, 0), 2)  # green: gt
    ...
    pred_RTs = np.array(pred_RTs)
    gt_scales = np.array(gt_scales)
    pred_scales = np.array(pred_scales)
    
    for gt_RT, pred_RT, gt_scale, pred_scale, cls, miss in zip(gt_RTs, pred_RTs, gt_scales, pred_scales, class_ids, misses):
        symmetry_flag = False
        class_name = synset_names[cls]
        
        
        if gt_RT.shape == (4, 4):
            gt_RT = np.expand_dims(gt_RT, axis=0)
        if pred_RT.shape == (4, 4):
            pred_RT = np.expand_dims(pred_RT, axis=0)
        if gt_scale.shape == (4, 4):
            gt_scale = np.expand_dims(gt_scale, axis=0)
        if pred_scale.shape == (4, 4):
            pred_scale = np.expand_dims(pred_scale, axis=0)
            
        if len(gt_RT) == 1 and len(pred_RT)==1:
            gt_RT = gt_RT.reshape(4,4)
            pred_RT = pred_RT.reshape(4,4)
            
            gt_scale = gt_scale.squeeze()
            pred_scale = pred_scale.squeeze()
            draw_img(gt_RT, pred_RT, gt_scale, pred_scale, class_name, save_path, draw_image)
        elif len(gt_RT) == len(pred_RT):
            for idx in range(len(gt_RT)):
                gt_RT_i = gt_RT[idx]
                pred_RT_i = pred_RT[idx]
                gt_scale_i = gt_scale[idx]
                pred_scale_i = pred_scale[idx]
                draw_img(gt_RT_i, pred_RT_i, gt_scale_i, pred_scale_i, class_name, save_path, draw_image)
        elif len(gt_RT) > len(pred_RT):
            for idx in range(len(gt_RT)):
                gt_RT_i = gt_RT[idx]
                gt_scale_i = gt_scale[idx]
            ...
            for idx in range(len(gt_RT)):
                ...
                draw_img(gt_RT_i, pred_RT_i, gt_scale_i, pred_scale_i, class_name, save_path, draw_image)
            start_idx = idx
            for idx_pred in range(start_idx, len(pred_RT)):
                gt_RT_i = gt_RT[start_idx]
                gt_scale_i = gt_scale[start_idx]
                
                pred_RT_i = pred_RT[idx_pred]
                pred_scale_i = pred_scale[idx_pred]
