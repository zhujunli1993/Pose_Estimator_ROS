...
import gpv_network.fs_net_repo.gcn3d as gcn3d
import torch
...
class FaceRecon_Contr(nn.Module):
    def __init__(self):
        super(FaceRecon_Contr, self).__init__()
    ...
    def forward(self,
                f_global: "tensor (bs, dim)",
                vertices: "tensor (bs, N, 3)",
                cat_id: "tensor (bs, 1)",
                ):
        """
        ...
        feat = torch.cat([fm_0, fm_1, fm_2, fm_3, fm_4, one_hot], dim=2) #feat: [128, 1024, 1286]
        ...
        feat_face_re = f_global.view(bs, 1, f_global.shape[1]).repeat(1, feat.shape[1], 1).permute(0, 2, 1) # f_global:[bs, f_global_dim (1024), N]
        # feat is the extracted per pixel level feature

        conv1d_input = feat.permute(0, 2, 1)  # (bs, fuse_ch, vertice_num)
        conv1d_out = self.conv1d_block(conv1d_input) # (bs, 256, vertice_num)

        recon = self.recon_head(conv1d_out)
        # average pooling for face prediction
        feat_face_in = torch.cat([feat_face_re, conv1d_out, vertices.permute(0, 2, 1)], dim=1) # bs*(1024+256+3)*N
        face = self.face_head(feat_face_in)
        
        return recon.permute(0, 2, 1), face.permute(0, 2, 1), feat_face_in
